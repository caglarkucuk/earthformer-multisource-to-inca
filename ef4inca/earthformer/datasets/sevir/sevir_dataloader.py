...
"""Below is the new constants for the INCA dataset!
...
Normalization concept of SEVIR repo is the exact opposite of mine!
My approach:
    norm -> (x` - mu)/sigma
    denorm -> x` * sigma + mu
    where mu and sigma are mean and std, respectively!

SEVIR approach:
    norm -> (x` + offset) * scale
    denorm -> x` / scale - offset
...
PREPROCESS_SCALE_SEVIR = {'vis': 1,  # Not utilized in original paper
                          'ir069': 1 / 1174.68,
                          'ir107': 1 / 2562.43,
                          'vil': 1 / 0.37410774996545293, # 1 / 47.54,
                          'lght': 1 / 0.60517}
PREPROCESS_OFFSET_SEVIR = {'vis': 0,  # Not utilized in original paper
                           'ir069': 3683.58,
                           'ir107': 1552.80,
                           'vil': -0.06588366464970631, # - 33.44,
                           'lght': - 0.02990}
